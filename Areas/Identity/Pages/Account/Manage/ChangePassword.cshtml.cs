...
public class ChangePasswordModel : PageModel
{
    ...
    private async Task UpdatePasswordInParameterStore(string oldPassword, string newPassword)
    {
        ...
        try
        {
            ...
            _logger.LogInformation($"Successfully updated password in AWS SSM Parameter Store for user {username}.");
            TempData["SuccessMessage"] = "Your password has been changed.";
        }
        catch (Exception ex)
        {
            _logger.LogError($"Error updating password in AWS SSM Parameter Store for user {username}: {ex.Message}");
        }
    }
